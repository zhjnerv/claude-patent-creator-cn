...
TOOL_ID = "patent-reviewer-CN-orchestrator"
TOOL_VERSION = "2.0.0"

REVIEW_TYPES = ("claims", "specification", "formalities")
CHECKER_SCRIPTS = {
    "claims": SKILLS_ROOT / "patent-claims-analyzer-CN" / "scripts" / "check_claims_cn.py",
    "specification": SKILLS_ROOT / "patent-specification-reviewer-CN" / "scripts" / "build_support_matrix_cn.py",
    "formalities": SKILLS_ROOT / "patent-formalities-reviewer-CN" / "scripts" / "check_formalities_cn.py",
}
...
def build_semantic_template(contract, prepare_id: str, binding: dict[str, str]) -> dict[str, Any]:
...
def command_prepare(args: argparse.Namespace) -> int:
    ...
    protected = [input_path, *paths.values()]
    ...
    if len(input_artifacts) > limits["verifier_max_artifacts"]:
        raise ResourceLimitError("输入 artifact 数超过上限")
    ...
    template = build_semantic_template(contract, prepare_id, binding)
...
def load_frozen_state(workspace: Path, contract) -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    ...
    # 输入和规则字节必须与冻结时一致，否则下游证据已经陈旧。
    for label, artifacts, expected in (
        ("输入", manifest["input_artifacts"], manifest["evidence_binding"]["input_set_sha256"]),
        ("规则", manifest["rule_sources"], manifest["evidence_binding"]["rule_set_sha256"]),
    ):
        for artifact in artifacts:
            path = Path(artifact["path"])
            if not path.is_file():
                raise OrchestrationError(f"{label}证据 {artifact['artifact_id']} 已不可读取：{path}")
            if sha256_bytes(path.read_bytes()) != artifact["sha256"]:
                raise OrchestrationError(
                    f"{label}证据 {artifact['artifact_id']} 字节已变化，冻结证据失效；必须重新运行 prepare"
                )
        if collection_digest(artifacts) != expected:
...
def command_finalize(args: argparse.Namespace) -> int:
    ...
    protected = [Path(item["path"]) for item in manifest["input_artifacts"]] + [args.review_input.resolve()]
