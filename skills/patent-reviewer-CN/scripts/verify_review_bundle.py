...
TOOL_ID = "patent-reviewer-CN-verifier"
TOOL_VERSION = "2.0.0"
...
def recompute_binding(
    ...
    for label, artifacts in (("输入", input_artifacts), ("规则", rule_sources)):
        for artifact in artifacts:
            path = Path(artifact["path"])
            try:
                raw = verifier.read_bytes(path, f"{label}工件 {artifact['artifact_id']}")
            ...
            if sha256_bytes(raw) != artifact["sha256"]:
                verifier.error(
                    ...
                    f"{label}工件 {artifact['artifact_id']} 的实际字节哈希与声明不一致，证据已陈旧",
                )
            elif len(raw) != artifact["byte_length"]:
                verifier.error(
                    CONTRACT_ERROR, "binding",
                    f"{label}工件 {artifact['artifact_id']} 的字节长度与声明不一致",
                )

    tool_set = sorted(
        [(item["tool_id"], item["tool_sha256"]) for item in checker_identities]
        + [(bundle["tool_identity"]["tool_id"], bundle["tool_identity"]["tool_sha256"])]
    )
    return {
...
def check_search_evidence_boundary(verifier: Verifier, bundle: dict[str, Any]) -> None:
    ...
    declared_artifacts = {item["artifact_id"] for item in bundle["input_artifacts"]}
...
def verify(verifier: Verifier, workspace: Path, bundle_path: Path) -> dict[str, Any]:
    ...
    for problem in contract.validate_bundle(bundle):
        verifier.error(CONTRACT_ERROR, "contract", problem)

    structurally_valid = not verifier.errors
    # 守恒检查始终执行：结构问题不应掩盖"原始 finding 在下游丢失"这类独立缺陷。
    for problem in contract.check_finding_conservation(raw_reports, bundle):
    ...
    if isinstance(bundle.get("input_artifacts"), list) and isinstance(bundle.get("rule_sources"), list) \
        ...
        for key, value in sorted(recomputed.items()):
            if bundle.get("evidence_binding", {}).get(key) != value:
                verifier.error(
                    CONTRACT_ERROR, "binding",
                    f"证据绑定 {key} 独立复算为 {value}，"
                    f"与 bundle 声明的 {bundle.get('evidence_binding', {}).get(key)} 不一致",
                )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return EXIT_VERIFICATION_FAILED
    except (VerificationInputError, OSError, UnicodeError, json.JSONDecodeError) as exc:
