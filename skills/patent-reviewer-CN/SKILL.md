...
1. **prepare**：冻结证据并运行三个真实检查器。
   ...
   产出 `raw/claims.json`、`raw/specification.json`、`raw/formalities.json`、`prepare-manifest.json` 和 `review-input-template.json`。任一原始报告不符合规范即整体失败，不产出工件。
...
3. **finalize**：守恒转换并构造 bundle。
   ...
   finalize 逐字节核对冻结证据的新鲜度；申请文件、规则矩阵、原始报告或工具身份中任何一项变化，都会使本轮证据失效并要求重新 prepare。
...
- **申请文件与程序 17 项**：核心文件与申请类型、请求书字段与主体身份、语言格式与签章、名称一致性、说明书结构、权利要求呈现与引用形式、摘要内容与字数、摘要附图指定、附图必要性与存在、图号标记与图形、序列表、生物材料保藏、遗传资源声明、优先权声明与文件、第二十四条声明与证明、分案程序、实审请求程序；
